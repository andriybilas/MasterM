...
{
	internal sealed class PageUrlUniqueRule : IEventListener<Page, ValidateEvent>
	{
		...
		private void CheckForSamePage(Page page)
		{
			...
			var samePage = Repository.Data.Get<Page>()
				.Where(x =>
				       ...
				       x.UrlName == page.UrlName)
				.FirstOrDefault()
				.Value;

			if (samePage != null)
			{
				throw new ValidationArgumentException(page, "PageUrlExistsException");
			}
		}
	}
}